...
if args.mode == "direct":
    target = p.loadURDF("target2/robot.urdf") # on load le modele physique croix r
    for joint in joints:
...
# target => dict qui recupere les infos moteur a un indice n +1 
while True:
    try:
        if sim.t > 1.0:
            if args.mode == "direct":
                #print(joints)
                for joint in joints:
                    # joint recuperer un dictionnaire avec les trois moteures 
                    # target["moteur1"] = recupere quoi ?
                    #
                    targets[joint] = p.readUserDebugParameter(sliders[joint])
                    # recupère les informations du moteure selectinner 
                    print()
                T = kinematics.computeDK(
                    -targets["motor1"], -targets["motor2"], targets["motor3"]
                )
                #T = model.direct(targets)
                # pk  recupere pas le y ? et bx == delta ?
                T[0] += bx
                T[2] += bz
                print(T[0] , T[2])
                print()
                ...
                #p.resetBasePositionAndOrientation(target, [T[0],0.0, T[2]] , [0, 0, 0, 1])
